            ...
            if ($scope.gaeSelected) {
                ...
                varClauses = [{
                    ...
                    },
                    {
                        ...
                        fieldName: 'cdCds',
                        operator: "=",
                        fieldValue: $scope.ordineMissioneModel.cdsSpesa
                    },
                    {
                        condition: 'AND',
                        fieldName: 'esercizioOriginale',
                        operator: "=",
                        fieldValue: $scope.ordineMissioneModel.esercizioOriginaleObbligazione
                    },
                    {
                        condition: 'AND',
                        fieldName: 'pgObbligazione',
                        operator: "=",
                        fieldValue: $scope.ordineMissioneModel.pgObbligazione
                    },
                    {
                        condition: 'AND',
                        fieldName: 'cdLineaAttivita',
                        operator: "=",
                        fieldValue: $scope.ordineMissioneModel.gaeSelected
                    }
                ];
            } else {
...
$scope.getDatiUo = function(uo) {
    ...
    $scope.uoValidazionieMissione = ProxyService.uoValidazionieMissione;
    ...
    $scope.onChangePartenzaDa = function() {
        if ($scope.ordineMissioneModel.partenzaDa === 'A') {} else {
    ...
    var deleteOrdineMissione = function() {
        $rootScope.salvataggio = true;
        OrdineMissioneService.delete({
                ids: $scope.ordineMissioneModel.id
            },
            function(responseHeaders) {
                $rootScope.salvataggio = false;
    ...
    $scope.doPrintOrdineMissione = function(idOrdineMissione) {
        ...
        $http.get('api/rest/ordineMissione/print/json', {
                params: {
                    idMissione: idOrdineMissione
                }
            })
            .success(function(data) {
    ...
    if (isInQuery() || ($scope.ordineMissioneModel != null && $scope.ordineMissioneModel.idMissione)) {
        ElencoOrdiniMissioneService.findById($scope.idMissione).then(function(data) {
            ...
            if (model) {
                ...
                $scope.getDatiUo($scope.ordineMissioneModel.uoSpesa);
            }
        });
    } else {
