...
missioniApp.factory('AutoNoleggioOrdineMissioneService', function($http) {
    return {
        findAutoNoleggio: function(idMissione) {
            var promise = $http.get('api/rest/ordineMissione/autoNoleggio/get', {
                params: {
        ...
        findSpostamenti: function(idAutoNoleggio) {
            var promise = $http.get('api/rest/ordineMissione/autoNoleggio/getSpostamenti', {
                params: {
                    idAutoNoleggio: idAutoNoleggio
                }
            }).then(function(response) {
                return response.data;
            });
...
missioniApp.controller('AutoNoleggioOrdineMissioneController', function($scope, $rootScope, $location, $routeParams, $sessionStorage, $http, $filter, AccessToken, AutoNoleggioOrdineMissioneService, ElencoOrdiniMissioneService, ui, COSTANTI, DateService) {
    ...
    $http.get('api/rest/ordineMissione/autoNoleggio/get', {
        params: {
        ...
        if (datiAutoNoleggioOrdineMissione.id === undefined) {
            inizializzaDati().then(function() {
                statoIniziale = angular.copy($scope.autoNoleggioOrdineMissioneModel); // Salva lo stato iniziale una volta caricati i dati
            });
    ...
    $scope.previousPage = function() {
        if (isStatoIniziale()) {
            ...
            } else if (isStatoIniziale() || $scope.spostamentiAutoNoleggio.length > 0) {
                parent.history.back();
            }
        }
    }
