...
missioniApp.factory('TaxiOrdineMissioneService', function($http) {
    return {
        findTaxi: function(idMissione) {
            var promise = $http.get('api/rest/ordineMissione/taxi/get', {
                params: {
        ...
        findSpostamenti: function(idTaxi) {
            var promise = $http.get('api/rest/ordineMissione/taxi/getSpostamenti', {
                params: {
                    idTaxi: idTaxi
                }
            }).then(function(response) {
...
missioniApp.controller('TaxiOrdineMissioneController', function($scope, $rootScope, $location, $routeParams, $sessionStorage, $http, $filter, AccessToken, TaxiOrdineMissioneService, ElencoOrdiniMissioneService, ui, COSTANTI, DateService) {
    ...
    $http.get('api/rest/ordineMissione/taxi/get', {
        params: {
        ...
        if (datiTaxiOrdineMissione.id === undefined) {
            inizializzaDati().then(function() {
                statoIniziale = angular.copy($scope.taxiOrdineMissioneModel); // Salva lo stato iniziale una volta caricati i dati
            });
    ...
    function isStatoIniziale() {
        return angular.equals($scope.taxiOrdineMissioneModel, statoIniziale);
    }
    ...
    $scope.previousPage = function() {
        if (isStatoIniziale()) {
            ...
            } else if (isStatoIniziale() || $scope.spostamentiTaxi.length > 0) {
                parent.history.back();
            }
        }
    }
