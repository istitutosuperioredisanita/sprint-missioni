...
//             return new Date(today.getFullYear(), meseAttuale - 5, 1);
  ...
  var recuperoTipoSpesa = function(inquadramento, data, nazione, trattamento){
      ...
      if (inquadramento) {
          tipoSpesaClauses.push({ condition: 'AND', fieldName: 'inquadramento', operator: "=", fieldValue: inquadramento });
      }

      if (data) {
          tipoSpesaClauses.push({ condition: 'AND', fieldName: 'data', operator: "=", fieldValue: data });
      }

      var objectPostTipoSpesa = {
          activePage: 0,
          maxItemsPerPage: COSTANTI.DEFAULT_VALUE_MAX_ITEM_FOR_PAGE_SIGLA_REST,
          clauses: tipoSpesaClauses
      };
