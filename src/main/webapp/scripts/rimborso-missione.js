...
missioniApp.controller('RimborsoMissioneController', function($rootScope, $scope, $routeParams, $sessionStorage, RimborsoMissioneService, OrdineMissioneService, ProxyService, ElencoOrdiniMissioneService, ElencoRimborsiMissioneService, AccessToken,
        ...
        for (var i = 0; i < $scope.elencoOrdiniMissione.length; i++) {
            if ($scope.elencoOrdiniMissione[i].id === idOrdineMissione) {
                ...
                if ($scope.rimborsoMissioneModel.uoSpesa) {
                    ...
                    $scope.restModulo($scope.rimborsoMissioneModel.anno, $scope.rimborsoMissioneModel.uoSpesa, $scope.rimborsoMissioneModel.pgProgetto);
    ...
    // flag che controlla l'esistenza del mandato
    $scope.mandatoExist = false;
    ...
    $scope.restCds = function(anno, cdsRich) {
        ...
        var objectPostCdsOrderBy = [{
            name: 'cd_proprio_unita',
            type: 'ASC'
        }];
        var objectPostCdsClauses = [{
            condition: 'AND',
        ...
        var objectPostCds = {
            ...
            orderBy: objectPostCdsOrderBy,
            clauses: objectPostCdsClauses
        }
        $http.post(urlRestProxy + app + '/', objectPostCds, {
            params: {
                proxyURL: url
            }
    ...
    $scope.restTipiSpesa = function(data, tipoMissione) {
        ...
        var objectPostTipiSpesaOrderBy = [{
            name: 'ds_ti_spesa',
            type: 'ASC'
        }];
        var objectPostTipiSpesaClauses = null;
        if (tipoMissione == "I") {
            objectPostTipiSpesaClauses = [{
                    condition: 'AND',
                    fieldName: 'ti_area_geografica',
                    operator: "!=",
                    fieldValue: 'E'
                },
                {
                    condition: 'AND',
                    fieldName: 'dt_inizio_validita',
                    operator: ">=",
                    fieldValue: data
                },
                {
                    condition: 'AND',
                    fieldName: 'dt_fine_validita',
                    operator: "<=",
                    fieldValue: data
                }
            ];
        ...
        var objectPostTipiSpesa = {
            activePage: 0,
            maxItemsPerPage: COSTANTI.DEFAULT_VALUE_MAX_ITEM_FOR_PAGE_SIGLA_REST,
            orderBy: objectPostTipiSpesaOrderBy,
            clauses: objectPostTipiSpesaClauses
        }
        $http.post(urlRestProxy + app + '/', objectPostTipiSpesa, {
            params: {
                proxyURL: url
            }
        }).success(function(data) {
    ...
    $scope.restNazioni = function() {
        ...
        var objectPostNazioneOrderBy = [{
            name: 'ds_nazione',
            type: 'ASC'
        }];
        var objectPostNazioneClauses = [{
            condition: 'AND',
            fieldName: 'ti_nazione',
            operator: "!=",
            fieldValue: 'I'
        }];
        var objectPostNazione = {
            activePage: 0,
            maxItemsPerPage: COSTANTI.DEFAULT_VALUE_MAX_ITEM_FOR_PAGE_SIGLA_REST,
            orderBy: objectPostNazioneOrderBy,
            clauses: objectPostNazioneClauses
        }
        $http.post(urlRestProxy + app + '/', objectPostNazione, {
            params: {
                proxyURL: url
            }
        }).success(function(data) {
            ...
            $http.post(urlRestProxy + app + '/', postModuli, {
                ...
                .then(function(response) {
                    ...
                    if (data && data.elements) {
                        ...
                        if (data.elements.length === 1) {
                            $scope.rimborsoMissioneModel.modulo = data.elements[0].cd_progetto;
                            $scope.rimborsoMissioneModel.modulo = data.elements[0].ds_progetto;
                        }
                    } else {
                        $scope.elencoModuli = [];
                    }
    ...
    $scope.restImpegno = function() {
        if ($scope.rimborsoMissioneModel.esercizioOriginaleObbligazione && $scope.rimborsoMissioneModel.pgObbligazione) {
            ...
            if ($scope.gaeSelected) {
                ...
                varClauses = [{
                    ...
                    },
                    {
                        ...
                        fieldName: 'cdCds',
                        operator: "=",
                        fieldValue: $scope.rimborsoMissioneModel.cdsSpesa
                    },
                    {
                        condition: 'AND',
                        fieldName: 'esercizioOriginale',
                        operator: "=",
                        fieldValue: $scope.rimborsoMissioneModel.esercizioOriginaleObbligazione
                    },
                    {
                        condition: 'AND',
                        fieldName: 'pgObbligazione',
                        operator: "=",
                        fieldValue: $scope.rimborsoMissioneModel.pgObbligazione
                    },
                    {
                        condition: 'AND',
                        fieldName: 'cdLineaAttivita',
                        operator: "=",
                        fieldValue: $scope.rimborsoMissioneModel.gaeSelected
                    }
                ];
            } else {
            ...
            $http.post(urlRestProxy + app + '/', postImpegno, {
                params: {
                    proxyURL: url
                }
            }).success(function(data) {
    ...
    $scope.inizializzaFormPerInserimento = function(account) {
        $scope.rimborsoMissioneModel = {
            nominativo: account.lastName + " " + account.firstName,
            qualificaRich: account.profilo,
            livelloRich: account.livello,
            codiceFiscale: account.codice_fiscale,
            dataNascita: account.data_nascita,
            luogoNascita: account.comune_nascita,
            validato: 'N',
            datoreLavoroRich: account.struttura_appartenenza,
            matricola: account.matricola,
            uoRich: ProxyService.buildUoRichiedenteSiglaFromUoSiper(account),
            cdsRich: $scope.estraiCdsRichFromAccount(account)
        };
    ...
    var deleteRimborsoMissione = function() {
        $rootScope.salvataggio = true;
        RimborsoMissioneService.delete({
                ids: $scope.rimborsoMissioneModel.id
            },
            function(responseHeaders) {
                $rootScope.salvataggio = false;
    ...
    $scope.doPrintRimborsoMissione = function(idRimborsoMissione) {
        ...
        $http.get('api/rest/rimborsoMissione/print/json', {
                params: {
                    idMissione: idRimborsoMissione
                }
            })
            .success(function(data) {
    ...
    var deleteAttachment = function(attachment, idRimborsoMissione) {
        $rootScope.salvataggio = true;
        var x = $http.delete('api/rest/deleteAttachment?id=' + attachment.id + '&idRimborsoMissione=' + attachment.idMissione);
            ...
            if ($scope.esisteRimborsoMissione()) {
                ...
                RimborsoMissioneService.modify($scope.rimborsoMissioneModel,
                    function(value, responseHeaders) {
                        ...
                        $rootScope.salvataggio = false;
                        $scope.disabilitaRimborsoMissione = impostadisabilitaRimborsoMissione();
                        ui.ok_message("Rimborso Missione modificata in bozza con successo");
                    },
                    function(httpResponse) {
                        $rootScope.salvataggio = false;
                    }
                ...
                RimborsoMissioneService.add($scope.rimborsoMissioneModel,
                    function(value, responseHeaders) {
                        ...
                        $scope.inizializzaFormPerModifica();
                        $scope.rimborsoMissioneModel.isFireSearchAttachments = false;
                        ui.ok_message("Rimborso Missione salvata in bozza con successo");
